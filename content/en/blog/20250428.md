...
The puzzle is fairly simple in principle: it's composed of 25 identical wooden
...
Since we're dealing with things that all ultimately decompose into 1x1x1 cube,
...
<img src="/images/puzzle_venn.svg" alt="Simple rendering of a piece composed of a few cubes put together" width="500"/> 
...
that each cube of the piece fit in the box:
...
and which results in a piece that fits the box:
...
allValidDispositions = filter isValidDispositions candidateDispositions
```

We're pretty much done for this first blog post, but I'd like to prepare for the
next one with one last step: our `Disposition` data type served us well, but all
...
[^1]: *Pedantic note: Mathematically speaking, a vector cannot have integer
      ...
      [module](https://en.m.wikipedia.org/wiki/Module_\(mathematics\)).
