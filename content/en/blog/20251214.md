...
* In the instance for `()`, `b` happens to be `()` already. So the second argument (which has type `f b`) actually has type `f ()`, exactly what we need. How lucky! Surely we won't be as lucky in the other instance?
...
data Expr unsafeFlag
  ...
  | Div unsafeFlag (Expr unsafeFlag) (Expr unsafeFlag)
```
What's changed?
