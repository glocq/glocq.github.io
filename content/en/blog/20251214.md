...
Here's how we can use the `TFlag` class to implement an type-safe optional type constructor:
```haskell
data Expr unsafeFlag
  = Literal Double
  | Plus (Expr unsafeFlaf) (Expr unsafeFlag)
  | Minus (Expr unsafeFlaf) (Expr unsafeFlag)
  | Times (Expr unsafeFlaf) (Expr unsafeFlag)
...
```
Or your function is able to deal with the unsafe version, in which case you will need to specify how to deal with the unsafe case. But thanks to the `whenFlag` method, your code with also work in the safe case:
```haskell
